...
Box2DFactory().then(box2D => {
  ...
  {
    const shape = new b2EdgeShape();
    shape.SetTwoSided(new b2Vec2(0, 29), new b2Vec2(64, 29));
    ground.CreateFixture(shape, 0);
  }
  // floor which boxes rest on
  {
    const shape = new b2EdgeShape();
    shape.SetTwoSided(new b2Vec2(0, 0), new b2Vec2(0, 29));
    ground.CreateFixture(shape, 0);
  }
  // floor which boxes rest on
  {
    const shape = new b2EdgeShape();
    shape.SetTwoSided(new b2Vec2(60, 0), new b2Vec2(60, 29));
    ground.CreateFixture(shape, 0);
  }
...
let pyramid = function() {
    ...
    const initPosition = (body, index) => {
        let temp = {x: 4 + sideLengthMetres*(Math.random()-0.5), y: -sideLengthMetres*index}
    ...
    for (let i = 0; i < boxCount; i++) {
        const bd = i % 2 ? 
        Bodies.rectangle(0,0,sideLengthMetres/2, sideLengthMetres/2) : 

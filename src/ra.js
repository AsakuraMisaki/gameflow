...
async function init(){
    ...
    // rigidBody collider
    let rigidBodyDesc = RAPIER.RigidBodyDesc.dynamic().setTranslation(5, gy-1);
    ...
    let bodyDesc = RAPIER.RigidBodyDesc.dynamic().setTranslation(8, gy-3);
    let body = world.createRigidBody(bodyDesc);
    // body.enableCcd(true);
    // body.sleep();
    let colliderDesc1 = RAPIER.ColliderDesc.convexHull(new Float32Array(points));
    ...
    convexgfx.beginFill(colorPalette[1], 1.0);
...
function polygenTest(){
  ...
  let gfx = ()=>{
    ...
    // convexgfx.stroke();
    window.scene.addChild(convexgfx);
  }
